...
sudo -H -u ec2-user bash -c "aws ssm get-parameter --name ${module.cluster.gitops_ssh_ssm_name} --with-decryption --query 'Parameter.Value' --region 'us-east-2' --output text > ~/.ssh/gitops_ssh.pem"
...
sudo -H -u ec2-user bash -c "ssh-keyscan -H git-codecommit.us-east-2.amazonaws.com >> ~/.ssh/known_hosts"
